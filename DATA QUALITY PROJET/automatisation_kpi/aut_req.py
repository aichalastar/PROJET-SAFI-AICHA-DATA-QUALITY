...
class AutomatisationRequetes:
    def __init__(self, annee=None, mois=None):
        # j'utiliserai la date actuelle si non spécifiée 
...
GROUP BY year, month, substr(msisdn, 4, 2);"""
    ...
    def generer_toutes_requetes(self):
        ...
        nom_fichier = f"requetes_mensuelles_{self.annee}_{self.mois:02d}.sql" # (je peux mettre aussi .hive" pour qu'il redirige directement vers hive )
...
def main():
    ...
    # Création de l'instance (j'utilise le mois/année actuel par défaut)
    # Ou spécifiez: AutomatisationRequetes(annee=2025, mois=6 (changeable))
...
# auto_juin = AutomatisationRequetes(annee=2025, mois=6 (changeable))
